...
async function seed() {
  ...
  // 6. Seed default Story Blocks
  console.log("\n📖 Seeding story blocks...");
  const existingStoryBlocks = await prisma.storyBlock.count();
  if (existingStoryBlocks === 0) {
    const storyBlocks = [
      {
        type: "hero",
    ...
    console.log(`   ✓ ${existingStoryBlocks} story blocks already exist — skipping`);
  }
