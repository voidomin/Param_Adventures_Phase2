...
2. **Resend webhook endpoint.** The webhook signature-verification code is live, but Resend won't send anything until an endpoint is registered on their side.
   - In the Resend Dashboard → Webhooks, add an endpoint pointed at `https://<production-domain>/api/webhooks/email`.
   - Copy the signing secret it gives you into Settings → Communications (see table above).
   - Only relevant if Resend is (or becomes) the active email provider — Zoho SMTP/API don't use this.
...
- [ ] Resend webhook endpoint registered in Resend dashboard
- [ ] Resend webhook signing secret entered
