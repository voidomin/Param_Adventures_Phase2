...
**Implementation status:** Release 1 (`v1.1.0`) and Release 2 (`v1.2.0`) shipped on branch `fix/release-1-correctness-hotfixes` — not yet merged to `main`. New environment variables introduced by Release 2, needed before deploy: `NEXT_PUBLIC_GOOGLE_CLIENT_ID` (Google Sign-In; feature no-ops if unset) and `ADMIN_IP_ALLOWLIST` (optional, opt-in admin network restriction; unset = no restriction). See `.env.example`.
...
- [x] **No admin 2FA / IP allowlisting.** SUPER_ADMIN and staff accounts — full access to customer PII and payment data — are password-only, with no MFA and no network restriction on `/admin`. This is the single account tier most worth hardening further. — **Fixed**: TOTP-based 2FA (enroll/confirm/disable + one-time backup codes) available to any account via Settings, enforced as a second login step once enabled; `ADMIN_IP_ALLOWLIST` env var (comma-separated IPs/CIDRs) restricts `/admin` and `/api/admin/*` in `src/proxy.ts` when configured — opt-in and empty by default so it can't accidentally lock out the only admin. **2FA is opt-in for every account, never required** — `twoFactorEnabled` defaults `false`, so a brand-new sign-up goes straight through to booking with no added step. A dismissible, lowest-priority suggestion banner (`TwoFactorPromptBanner`) nudges users toward turning it on, deferring to email-verification/profile-completion prompts and permanently dismissible (localStorage, not per-session) so it doesn't nag.
