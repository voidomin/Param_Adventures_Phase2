...
describe("auth library", () => {
  ...
  describe("password hashing", () => {
    it("calls bcrypt.hash with correct rounds", async () => {
      vi.mocked(bcrypt.hash).mockResolvedValue("hashed" as unknown);
  ...
  describe("JWT tokens", () => {
    it("generates an access token", () => {
      vi.mocked(jwt.sign).mockReturnValue("access-token" as unknown);
    ...
    it("generates a refresh token", () => {
      vi.mocked(jwt.sign).mockReturnValue("refresh-token" as unknown);
    ...
    it("verifies a valid access token", async () => {
      ...
      vi.mocked(jwt.verify).mockReturnValue(payload as unknown);
    ...
    it("returns null for invalid user or status", async () => {
      vi.mocked(jwt.verify).mockReturnValue({ userId: "user1", roleName: "ADMIN", tokenVersion: 1 } as unknown);
      vi.mocked(prisma.user.findUnique).mockResolvedValue({
        status: "BANNED",
        tokenVersion: 1,
        deletedAt: null,
    ...
    it("returns null for malformed token payload", async () => {
      vi.mocked(jwt.verify).mockReturnValue({ userId: "user1" } as unknown); // missing fields
    ...
    it("returns null for invalid refresh token type", () => {
      vi.mocked(jwt.verify).mockReturnValue({ userId: "user1", type: "access" } as unknown);
