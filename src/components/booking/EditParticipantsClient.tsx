...
import { MapPin, User, X, AlertTriangle, Edit2, ShieldAlert } from "lucide-react";
  ...
  const handleCancelParticipant = async () => {
    ...
    try {
      const res = await fetch(`/api/bookings/${bookingId}/cancel-participants`, {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          participantIds: [cancellingParticipant.id],
      ...
      if (data.refundAmount > 0) {
        alert(`Slot cancelled successfully. Refund amount of ₹${data.refundAmount} is pending support review.`);
      } else {
                  ...
                  <td className="px-6 py-4 text-right">
                    {!p.isCancelled && (
                      <div className="flex justify-end gap-2">
                        ...
                        <button
                          type="button"
                          onClick={() => setCancellingParticipant(p)}
          ...
          <div className="bg-card border border-border w-full max-w-md rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center p-6 border-b border-border">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-lg font-bold text-foreground">Cancel Guest Slot</h3>
              </div>
              <button
                type="button"
                onClick={() => setCancellingParticipant(null)}
                className="text-foreground/40 hover:text-foreground transition-colors"
              >
                <X className="w-6 h-6" />
              </button>
            ...
            <div className="p-6 space-y-4">
              {activeParticipants.length === 1 ? (
                <div className="bg-red-500/10 border border-red-500/20 text-red-500 text-xs font-semibold rounded-xl p-4 flex gap-2">
                  ...
                  <p>
                    <strong>Warning:</strong> Since this is the last remaining guest slot in this booking,
                    cancelling this guest will cancel the entire booking request.
                  </p>
                </div>
              ) : (
                <p className="text-sm text-foreground/80 leading-relaxed">
                  You are cancelling the slot for <strong>{cancellingParticipant.name}</strong>.
            ...
            <div className="flex gap-3 p-6 border-t border-border bg-muted/20">
              ...
              >
                {isCancelSubmitting ? "Processing…" : "Cancel Slot"}
              </button>
            </div>
          </div>
        </div>
